        ...
        {filteredBlocks.map((block) => {
          switch (block.type) {
            ...
            case "text": {
              ...
              if (!text) return null;
              return (
                <section key={block.id} className="text-center text-neutral-200 whitespace-pre-wrap">
                  {text}
                </section>
              );
            }
          ...
          <section className="space-y-3 pt-2">
            {(firstLinks.content?.items ?? []).map(
              (item: { title?: string; url?: string }, idx: number) => {
                ...
                const url = item?.url ?? "#";
                return (
                  <LinkButton key={idx} href={url} label={title} buttonStyle={site.button_style as any} />
                );
              }
            )}
          </section>
